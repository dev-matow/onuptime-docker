...
async function main() {
  ...
  const rows = ids.map((id) => {
    ...
    return {
      ...
      // from the kind, which is the same derivation
      // `probeExecutability` makes at runtime: an agent dials a target,
      // and three of the forty types have none. Recorded here so the
      // matrix answers "is every type supported remotely" without
      // anybody having to know that "no" is sometimes the correct
      // answer.
      remoteProbe: spec.kind === "active" ? true : "n/a",
    };
  });
...
${rows.length} types: ${rows.filter((r) => r.remoteProbe === true).length} run on probes, ${rows.filter((r) => r.remoteProbe !== true).length} run only on the controller.
